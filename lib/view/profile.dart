...
class _MyProfileState extends State<MyProfile> {
  ...
  Future Auto_check_permission() async {
    ...
    await api_controller()
        .Viewer_permission_check(User_token)
        .then((value) => Viewer_permission_info = value.body);
    setState(() {
      if (Viewer_permission_info.length > 0) {
        Viewer_permission_info;
      } else {
        Viewer_permission_info = "ไม่พบข้อมูล";
      }
    });
            ...
            Container(
              ...
              child: Column(
                children: <Widget>[
                  ...
                  Text("สิทธิ์ผู้ใช้งาน ระดับ Viewer =>" +
                      Viewer_permission_info),
                  Text(
